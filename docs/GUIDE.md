...
* Fresh installs promote the installer-created machine user into the initial CrateOS admin operator; if first-use/operator state is incomplete later, the local console should enter the CrateOS primer and `crateos bootstrap <name>` remains the manual local recovery path
...
* The CrateOS primer is the first-use installer/manager surfaced through that console takeover path; it must complete before the normal console menu unlocks and is responsible for persisting machine identity, repairing local takeover, and provisioning the first local admin
